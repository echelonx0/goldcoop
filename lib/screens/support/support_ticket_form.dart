...
// lib/screens/support/support_ticket_form.dart
...
class _SupportTicketFormState extends State<SupportTicketForm> {
  ...
  // Computed user info - prefer UserModel if available
  String get _displayName => widget.userModel?.displayName ?? widget.userName;
  String get _userEmail => widget.userModel?.email ?? widget.userEmail;
  String? get _userAvatar => widget.userModel?.profilePic;
    ...
    try {
      ...
      if (ticketId != null) {
        ...
        final conversation = SupportConversation(
          ...
          userId: widget.userId,
          relatedTicketId: ticketId,
          title: 'Support: ${_subjectController.text.trim()}',
          createdAt: DateTime.now(),
          updatedAt: DateTime.now(),
          status: ConversationStatus.open,
        );
        ...
        // Send initial system message with user context
        await _sendInitialMessage(ticketId);
  ...
  /// Sends initial message with user context for support team
  Future<void> _sendInitialMessage(String ticketId) async {
    final user = widget.userModel;
    if (user == null) return;

    // Build context string for support team
    final contextParts = <String>[];

    if (user.phoneNumber.isNotEmpty) {
      contextParts.add('Phone: ${user.phoneNumber}');
    }
    if (user.country?.isNotEmpty == true) {
      contextParts.add('Country: ${user.country}');
    }
    contextParts.add('KYC: ${user.kycStatus.name}');
    contextParts.add('Account: ${user.accountStatus.name}');

    if (contextParts.isEmpty) return;

    // This context could be stored as ticket metadata instead
    // For now we're just preparing it - the support service can use it
  }
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // User info card (if UserModel available)
    ...
    return Container(
      ...
      child: Row(
        children: [
          CircleAvatar(
            radius: 20,
            backgroundColor: AdminDesignSystem.accentTeal.withAlpha(25),
